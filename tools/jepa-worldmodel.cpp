...
//
// --- Planning (--plan, the V-JEPA 2-AC CEM loop) ------------------------------------------------
//
//   jepa-worldmodel --plan -m vjepa2-ac-vitg-f16.gguf --image ctx.png --goal goal.png
//                   [--state 'x,y,z,rx,ry,rz,g'] [--samples K] [--topk N] [--cem-steps S]
//                   [--horizon H] [--maxnorm M] [--cem-seed S | --noise-npy n.npy] [-o plan.npy]
//       Encodes the current frame and the goal frame once into a cached planning context, then runs
//       Meta's cross-entropy method (notebooks/utils/mpc_utils.py::cem) over it: each iteration
//       samples K action trajectories, rolls them all out in one batched graph per horizon step,
//       scores the final frame with the L1 energy and moves the sampling distribution towards the
//       topk elites. Prints the best energy per iteration and the chosen action sequence.
//       --noise-npy replays recorded standard-normal draws ([cem_steps, H, K, 4]) so a run can be
//       compared against a PyTorch one; without it the draws come from --cem-seed.
//
// --- LeWM (the default) ------------------------------------------------------------------------
//
...
static void usage(const char * a0) {
    fprintf(stderr,
        ...
        "  --plan            V-JEPA 2-AC CEM planning: --image + --goal -> an action sequence\n"
        "  --samples K       CEM population per iteration (default 100)\n"
        "  --topk N          CEM elites (default 10)      --cem-steps S   iterations (default 10)\n"
        "  --maxnorm M       |dx|,|dy|,|dz| clamp (default 0.05)   --cem-seed S   RNG seed\n"
        "  --noise-npy F     replay recorded draws [cem_steps, H, K, 4] instead of sampling\n"
...
static int ac_plan(jepa_context * ctx, jepa_model * model, const std::vector<std::string> & images,
                   const std::string & goal_path, jepa_ac_cem_params q, const std::string & noise_npy,
                   const std::vector<float> & state0, const std::string & out_path) {
    const int64_t D = jepa_model_embed_dim(model);
    const int64_t HW = jepa_ac_tokens_per_frame(model);
    const int A = jepa_ac_action_dim(model), S = jepa_ac_state_dim(model);
    if (images.empty() || goal_path.empty()) {
        fprintf(stderr, "--plan needs --image (the current frame) and --goal\n");
        return 2;
    }
    printf("model: %s (%s, %s) | device %s | threads %d | tokens/frame %lld\n",
           jepa_model_name(model), jepa_model_family(model), jepa_model_file_type_name(model),
           jepa_model_device_name(model), jepa_context_n_threads(ctx), (long long) HW);

    std::vector<float> cur, goal;
    if (!ac_encode_frame(ctx, model, images[0], cur)) return 2;
    const double enc_ms = jepa_context_last_compute_ms(ctx);
    if (!ac_encode_frame(ctx, model, goal_path, goal)) return 2;
    printf("context: %s | goal: %s  (%lld tokens x %lld, %.1f ms per encode)\n",
           images[0].c_str(), goal_path.c_str(), (long long) HW, (long long) D, enc_ms);

    std::vector<float> noise;
    if (!noise_npy.empty()) {
        npy::Array a = npy::load(noise_npy);
        if (a.shape.size() != 4) {
            fprintf(stderr, "%s must be [cem_steps, horizon, samples, 4]\n", noise_npy.c_str());
            return 2;
        }
        q.cem_steps = (int) a.shape[0];
        q.horizon   = (int) a.shape[1];
        q.samples   = (int) a.shape[2];
        noise = a.to_f32();
        printf("replaying recorded draws: %d iterations x H=%d x K=%d\n", q.cem_steps, q.horizon, q.samples);
    }
    std::vector<float> st = state0;
    st.resize((size_t) S, 0.0f);

    jepa_ac_context * h = jepa_ac_context_new(ctx, cur.data(), 1, nullptr, st.data());
    if (!h) return 2;
    std::vector<float> plan((size_t) q.horizon * A), best((size_t) q.cem_steps);
    printf("CEM: K=%d topk=%d iterations=%d horizon=%d maxnorm=%.3f momentum(mean/std)=%.2f/%.2f\n",
           q.samples, q.topk, q.cem_steps, q.horizon, q.maxnorm, q.momentum_mean, q.momentum_std);
    const double t0 = now_ms();
    const int rc = jepa_ac_plan(ctx, h, goal.data(), &q, noise.empty() ? nullptr : noise.data(),
                                plan.data(), best.data());
    const double wall = now_ms() - t0;
    ...
    for (int i = 0; i < q.cem_steps; i++) printf("%-10d %14.6f\n", i, best[(size_t) i]);
    printf("\nplan (%d step%s, %d-d actions):\n", q.horizon, q.horizon == 1 ? "" : "s", A);
    for (int hh = 0; hh < q.horizon; hh++) {
        printf("  step %d:", hh);
        for (int j = 0; j < A; j++) printf(" %8.5f", plan[(size_t) hh * A + j]);
        printf("\n");
    }
    printf("\n%.1f ms for %d iterations (%.1f ms each, %d rollouts of %d steps -> %.2f ms per candidate-step)\n",
           wall, q.cem_steps, wall / q.cem_steps, q.cem_steps * q.samples, q.horizon,
           wall / ((double) q.cem_steps * q.samples * q.horizon));
    if (!out_path.empty()) {
        npy::save_f32(out_path, { q.horizon, A }, plan.data());
        printf("wrote %s [%d, %d] float32\n", out_path.c_str(), q.horizon, A);
    }
    return 0;
}
...
int main(int argc, char ** argv) {
    ...
    bool ac_mode = false, plan_mode = false;
    int horizon = 0;
    std::string noise_npy;
    jepa_ac_cem_params cem = jepa_ac_cem_default_params();
    cem.cem_steps = 10;   // the notebook's mpc_args, not the signature's 100
    cem.horizon = 2;
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (a == "--plan") { plan_mode = true; ac_mode = true; }
        else if (a == "--samples") cem.samples = atoi(next());
        else if (a == "--topk") cem.topk = atoi(next());
        else if (a == "--cem-steps") cem.cem_steps = atoi(next());
        else if (a == "--maxnorm") cem.maxnorm = (float) atof(next());
        else if (a == "--cem-seed") cem.seed = (uint32_t) strtoul(next(), nullptr, 10);
        else if (a == "--noise-npy") noise_npy = next();
    ...
    if (ac_mode) {
        ...
        if (plan_mode) {
            if (horizon > 0) cem.horizon = horizon;
            std::vector<float> st0;
            if (!state_str.empty()) parse_actions(state_str, jepa_ac_state_dim(model), st0);
            const int rc = ac_plan(c, model, images, goal_path, cem, noise_npy, st0, out_path);
            jepa_context_free(c);
            jepa_model_free(model);
            return rc;
        }
