...
| classification with the attentive-pool head (SSv2) | **q8_0** | logits cosine 0.9998, top-1 and top-5 identical to the reference on the fixtures; q4 still matched top-1 here but top-5 overlap fell to 0.9 and the logits moved by 17 % (`rel_max`) — do not trust q4 near a decision boundary |
| dense per-token features (segmentation-style use of `last_hidden_state`, per-token retrieval) | **q8_0**, at most q6_k; prefer f16 for V-JEPA 2 ViT-L | q6_k keeps the mean token cosine >= 0.998 but the worst tokens go to 0.91 on ViT-H; q5 and below distort individual tokens badly (worst 0.5-0.7); V-JEPA 2 ViT-L already has 3 % of tokens below 0.99 at q8_0 |
