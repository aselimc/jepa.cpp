...
#   --results-json FILE  machine-readable summary committed with the docs
#                        (default tests/results/benchmarks.json; "" to skip)
...
RESULTS_JSON="$ROOT/tests/results/benchmarks.json"
...
# ---------------------------------------------------------------------------------------------
# Everything the metadata needs is passed as argv (the heredoc delimiter is quoted, so the Python
# below is never re-expanded by the shell): a compiler banner or a --note carrying a quote, a
# backslash or a newline used to be spliced straight into the source and could break it — or, worse,
# execute. A failure here aborts the sweep: the tables are unusable without the box description.
write_meta() {  # $1 = "start" | "end"
    ...
    "$PYTHON" - "$OUT/meta.json" "$1" "$cpu" "$cores" "$mem" "$cxx" "$ggml_commit" \
                 "${llamafile:-unknown}" "$THREADS" "$REPEAT" "$WARMUP" "$NOTE" <<'PY'
import json, os, sys, datetime
(path, phase, cpu, cores, mem, cxx, ggml_commit, llamafile,
 threads, repeat, warmup, note) = sys.argv[1:13]
...
if phase == "start":
    sessions.append({"threads": threads, "repeat": int(repeat), "warmup": int(warmup),
...
# ---------------------------------------------------------------------------------------------
# --ftype-label carries the type asked for at conversion time (the filename suffix). general.file_type
# is the most common stored tensor type, and a q4_k mix falls back to q4_0 wherever a row is not a
# multiple of the 256-element super-block — for the small models that is the majority of tensors, so
# the file reads as q4_0 and without this its rows would be labelled, and merged with, the real q4_0 ones.
run_bench() {  # $1 gguf  $2 label  $3 ftype  $4 mode  $5 tag  $6... extra args
    local gguf="$1" label="$2" ftype_label="$3" mode="$4" tag="$5"; shift 5
    local stem json
    stem="$(basename "$gguf" .gguf)"
    json="$OUT/${stem}__${mode}__${tag}__t${THREADS//,/_}.json"
    local cmd=("$BENCH" -m "$gguf" --mode "$mode" --label "$label" --ftype-label "$ftype_label"
               --threads "$THREADS" --repeat "$REPEAT" --warmup "$WARMUP" --steps "$STEPS"
               --json "$json" "$@")
...
if [ "$FAILURES" -gt 0 ]; then
    echo "$FAILURES config(s) failed — the document will be missing their rows:" >&2
    printf '  - %s\n' "${FAILED_CONFIGS[@]}" >&2
fi
echo "JSONs in $OUT" >&2
...
GEN_RC=0
if [ "$GEN_DOC" = 1 ]; then
    gen=("$PYTHON" "$ROOT/scripts/gen_benchmarks_md.py" --bench-dir "$OUT"
         --ref-dir "$ROOT/tests/fixtures/ref" --parity "$ROOT/docs/parity.md" -o "$DOC")
    [ -z "$RESULTS_JSON" ] || gen+=(--results-json "$RESULTS_JSON")
    "${gen[@]}" || GEN_RC=$?
